...
public class HappyPath : AppPageTest
{
  ...
  [Fact]
  public async Task EditUserRolesRemovesRole()
  {
    // Arrange - create admin, invite user, give them ADMIN + AUTHOR roles via API
    var admin = await Api.CreateUserAsync();
    var invited = await Api.InviteUserAsync(admin.Token);
    var invitedUserId = await Api.GetUserIdByEmailAsync(admin.Token, invited.Email);
    await Api.UpdateUserRolesAsync(admin.Token, invitedUserId, ["ADMIN", "AUTHOR"]);
    ...
    // Assert - ADMIN should no longer be in the roles
    await Expect(Pages.UsersPage.GetUserRowByUsername(invited.Email)).Not.ToContainTextAsync("ADMIN");
    await Expect(Pages.UsersPage.GetUserRowByUsername(invited.Email)).ToContainTextAsync("AUTHOR");
  }
