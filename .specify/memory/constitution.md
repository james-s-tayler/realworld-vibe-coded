...
A Medium.com clone ("Conduit") — social blogging platform implementing the RealWorld spec.

- **Backend**: .NET 10 / FastEndpoints / MediatR CQRS / EF Core + SQL Server / Serilog
- **Frontend**: React + Vite + TypeScript / Carbon Design System
- **Build**: Nuke build system (`./build.sh`) — never invoke `dotnet` directly
- **Tests**: xUnit (backend), Vitest (frontend), Playwright E2E, Postman/Newman (API contract)
...
| `SPEC-REFERENCE.md` | API | Complete API spec — the source of truth for what to build |
