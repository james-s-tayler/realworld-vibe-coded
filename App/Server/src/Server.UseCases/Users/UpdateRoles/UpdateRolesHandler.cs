...
#pragma warning restore PV014
  {
    ...
    var hasOwner = currentRoles.Contains(DefaultRoles.Owner);

    // Compute roles to remove (exclude OWNER from removal)
    var rolesToRemove = currentRoles
      .Where(r => r != DefaultRoles.Owner && !request.Roles.Contains(r))
      .ToList();
