...
public class ApplicationUsersQuery : IQueryApplicationUsers
{
  ...
  {
    // Load paginated users first, then load roles separately.
    // The AsEnumerable() subquery pattern doesn't materialize role names
    // correctly when combined with Skip/Take pagination on SQL Server.
