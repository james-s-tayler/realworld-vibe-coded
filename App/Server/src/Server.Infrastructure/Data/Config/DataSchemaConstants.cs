namespace Server.Infrastructure.Data.Config;

public static class DataSchemaConstants
{
  public const int DEFAULT_NAME_LENGTH = 100;
  public const int EMAIL_LENGTH = 255;
  public const int USERNAME_LENGTH = 100;
  public const int HASHED_PASSWORD_LENGTH = 255;
  public const int BIO_LENGTH = 1000;
  public const int IMAGE_URL_LENGTH = 500;

  // Article constants
  public const int ARTICLE_SLUG_LENGTH = 255;
  public const int ARTICLE_TITLE_LENGTH = 255;
  public const int ARTICLE_DESCRIPTION_LENGTH = 1000;
  public const int ARTICLE_BODY_LENGTH = 10000;

  // Tag constants
  public const int TAG_NAME_LENGTH = 100;
}
